...
// 批量放生选中的品质
const selectedRarities = ref(JSON.parse(localStorage.getItem('selectedRarities') || '[]'))

// 保存选中的品质到本地存储
const saveSelectedRarities = () => {
  localStorage.setItem('selectedRarities', JSON.stringify(selectedRarities.value))
}

// 批量放生函数
const batchReleasePets = () => {
  if (selectedRarities.value.length === 0) {
    message.warning('请至少选择一个品质')
    return
  }
  playerStore.items = playerStore.items.filter(item =>
    item.type !== 'pet' ||
    item.id === playerStore.activePet?.id ||
    !selectedRarities.value.includes(item.rarity)
  )
  showBatchReleaseConfirm.value = false
  message.success(`已放生选中品质的未出战灵宠`)
}
...
const options = Object.entries(petRarities).map(([key, rarity]) => ({
  label: rarity.name,
  value: key,
              ...
              <n-space style="margin: 16px 0">
                <n-checkbox
                  ...
                  @update:checked="(checked) => {
                    if (checked) {
                      if (!selectedRarities.includes(key)) {
                        selectedRarities.push(key)
                      }
                    } else {
                      const index = selectedRarities.indexOf(key)
                      if (index > -1) {
                        selectedRarities.splice(index, 1)
                      }
                    }
                    saveSelectedRarities()
                  }"
                >
                  <span :style="{ color: rarity.color }">{{ rarity.name }}</span>
                </n-checkbox>
              </n-space>
              <p>此操作不可撤销，确定要放生选中品质的未出战灵宠吗？</p>
